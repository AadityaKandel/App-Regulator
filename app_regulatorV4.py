...
function Get-MainExecutable($installPath) {
...
$installedApps = foreach ($path in $registryPaths) {
    Get-ItemProperty $path -ErrorAction SilentlyContinue | Where-Object { $_.DisplayName } | ForEach-Object {
        $mainExe = Get-MainExecutable $_.InstallLocation
        [PSCustomObject]@{
            Name           = $_.DisplayName
...
def scan_apps():
    global formatted_apps, exe_name, details
    btn3.config(text="Scanning....",bg="yellow",fg="black",state=DISABLED)
    try:
        result = subprocess.run(
            ["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", scan_app_script],
            capture_output=True,
            text=True
        )
        ...
        if result.returncode == 0:
            ...
            # Formatting into a list of lists: [[Name, MainExecutable], ...]
            formatted_apps = [[app["Name"], app["MainExecutable"]] for app in app_details]
    ...
    # Save Scanned Apps
    details=formatted_apps
    encrypt_everything()
    f = open('scanned_apps.json','wb')
    details = details.encode('utf-8') # Convert to bytes
    f.write(details)
    f.close()
    details = [] # Initialize Details Back To Normal

    btn3.config(text="Re-Scan Apps",bg="black",fg="white",state=NORMAL)

def load_scanned_apps():
    global formatted_apps, load_scanned_apps
    if os.path.exists('scanned_apps.json'):
        f = open('scanned_apps.json','rb')
        file_content = f.read()
        f.close()
        ...
        try:
            ...
            formatted_apps = ast.literal_eval(file_content)
            load_scanned_apps=True
        except:
            load_scanned_apps=False
            return

        for i in range(len(formatted_apps)):
            l2.insert(i,f"{formatted_apps[i][0]}")

        btn3.config(text="Re-Scan Apps")

def initiate_scan():
    t2 = threading.Thread(target=scan_apps)
    t2.start()
...
def add_scanned_targets():
    ...
    if selected_targets == ():
        if l2.get(0) == '':
            return tmsg.showinfo('Info','You first need to scan apps, select them & click here to put them as your target...')
        return tmsg.showerror('Error','No Targets Selected')

    for x in selected_targets:
        initial_name = formatted_apps[x][1] # Initial Directory of Selected App
        final_name = initial_name.replace('\\','/')
        exe_name = app_full_name(final_name)

        # Check if the target already exists
        if exe_name in target_name:
            tmsg.showinfo('Info',f'{exe_name} is already selected')
        else:
...
def app_full_name(path):
    global exe_name
    backslash_index = path.rindex('/')
    exe_name=path[backslash_index+1::]
    return exe_name
    ...
    if question=="yes":
        ...
        if a =="":
            ...
            for x in range(len(a)):
                exe_name = app_full_name(a[x].name)
...
def initiate_taskkill():
    ...
    if len(target_name) == 0:
        ...
        btn_main.config(text="Stop", bg="green", command=stop_attack)
        disable_everything()
...
def stop_attack():
    ...
    abort=True
    loop=False
    btn_main.config(text="Block", bg="red", fg="black",command=initiate_taskkill)
...
l2.config(yscrollcommand=scrollbar2.set)
scrollbar.config(command=l1.yview)
scrollbar2.config(command=l2.yview)
